...
try {
  for (const lang of ['fr', 'en']) {
    for (const audience of ['agency', 'visitor']) {
      const email = contactEmail({ lang, audience, name: 'Camille Dupont', email: 'camille@example.com', company: 'Atelier & Co', topic: lang === 'fr' ? 'Développement web' : 'Web development', from: 'Thermidor <contact@thermidor-agence-web.fr>', message: lang === 'fr' ? 'Bonjour,\n\nNous aimerions repenser le site de notre entreprise pour mieux présenter nos services et nos réalisations.\n\nSeriez-vous disponibles pour en discuter ?\n\nMerci et à bientôt,\nCamille' : 'Hello,\n\nWe would like to redesign our company website to better showcase our services and projects.\n\nWould you be available to discuss it?\n\nThank you,\nCamille' });
      const composed = await transport.sendMail(email);
      await writeFile(`artifacts/email/contact-${audience}-${lang}.eml`, composed.message);
      ...
      await writeFile(`artifacts/email/contact-${audience}-${lang}.html`, preview);
      ...
      for (const [name, width] of [['desktop', 760], ['mobile', 390]]) {
        ...
        await page.screenshot({ path: `artifacts/email/contact-${audience}-${lang}-${name}.png`, fullPage: true });
      }
      await page.close();
    }
  }
